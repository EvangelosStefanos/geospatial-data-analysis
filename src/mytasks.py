...
class MySemanticSegmentationTask(BaseTask):
        ...
        * :class:`~torchmetrics.classification.MulticlassJaccardIndex`: Intersection
        ...
        metrics = MetricCollection(
            [
                ...
                MulticlassJaccardIndex(
                    num_classes=num_classes, ignore_index=ignore_index, average=average
                ),
                MulticlassF1Score(
                    num_classes=num_classes, ignore_index=ignore_index, average=average
                ),
    ...
    def stack_images(self, batch):
        """Stack two images.

        Args:
            batch (dict): Dictionary with keys {"image1", "image2", "mask"} or {"image", "mask"}

        Returns:
            dict: Dictionary with keys {"image", "mask"}
        """
        ...
        stacked = {  # [B, 2 * C, H, W]
            "image": torch.cat(
                tensors=(
                    batch["image1"].to(self.device),  # [B, C, H, W]
                    batch["image2"].to(self.device),  # [B, C, H, W]
                ),
                dim=1,
            ),
    ...
    def validation_step(
        ...
        if (
            ...
            batch["image1"] = batch["image"][:, :3, :, :]
            batch["image2"] = batch["image"][:, 3:, :, :]
    ...
    def test_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> None:
        ...
        y_hat = self(x)
        loss = self.criterion(y_hat, y)
        self.log("test_loss", loss, batch_size=batch_size, prog_bar=False)
        self.test_metrics(y_hat, y)
        self.log_dict(self.test_metrics, batch_size=batch_size, prog_bar=False)
